...
const metaTitle = "Rancher 2.5 文档 | Rancher";
function findAndAppendSubGroups(all, metadata, baseUrl, subItems) {
    subItems.forEach((sub) => {
        if (typeof sub === "string") {
            const label = metadata.docs.rancher2_5[sub];
            if (label) {
                all.push({
                    label,
...
function getToc(sidebars, metadata, baseUrl) {
    const out = [];
    const docs = sidebars.rancher2_5;
    Object.keys(docs).forEach((categoryKey) => {
        ...
        const description = metadata.categories.rancher2_5[categoryKey];
        if (description) {
            out.push({
                key: categoryKey,
                description,
...
function Home() {
    ...
    const title = "Rancher 2.5 中文文档（alpha）";
    return (
