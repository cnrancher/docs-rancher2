...
const metaTitle = " Harvester文档 | Rancher";
function findAndAppendSubGroups(all, metadata, baseUrl, subItems) {
    subItems.forEach((sub) => {
        if (typeof sub === "string") {
            const label = metadata.docs.harvester[sub];
            if (label) {
                all.push({
                    label,
...
function getToc(sidebars, metadata, baseUrl) {
    ...
    Object.keys(docs).forEach((categoryKey) => {
        ...
        const description = metadata.categories.harvester[categoryKey];
        if (description) {
            out.push({
                key: categoryKey,
                description,
