...
const keywords = ["Rancher文档", "k8s文档", "容器管理平台", "Kubernetes集群", "Rancher Desktop"];
const metaTitle = "Rancher Desktop文档 | Rancher";
function findAndAppendSubGroups(all, metadata, baseUrl, subItems) {
    subItems.forEach((sub) => {
        if (typeof sub === "string") {
            const label = metadata.docs.rancherdesktop[sub];
            if (label) {
                all.push({
                    label,
...
function getToc(sidebars, metadata, baseUrl) {
    const out = [];
    const docs = sidebars.rancherdesktop;
    Object.keys(docs).forEach((categoryKey) => {
        ...
        const description = metadata.categories.rancherdesktop[categoryKey];
        if (description) {
            out.push({
                key: categoryKey,
                description,
...
function Home() {
    ...
    const title = "Rancher Desktop 中文文档";
    return (
