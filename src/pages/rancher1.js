...
const metaTitle = "Rancher 1.x 文档 | Rancher";
function findAndAppendSubGroups(all, metadata, baseUrl, subItems) {
    subItems.forEach((sub) => {
        if (typeof sub === "string") {
            const label = metadata.docs.rancher1[sub];
            if (label) {
                all.push({
                    label,
...
function getToc(sidebars, metadata, baseUrl) {
    ...
    Object.keys(docs).forEach((categoryKey) => {
        ...
        const description = metadata.categories.rancher1[categoryKey];
        if (description) {
            out.push({
                key: categoryKey,
                description,
            ...
            <header className={classnames("hero", styles.heroBanner)}>
                <div className="container">
                    ...
                    <div className="text-xs text-gray">
                        Rancher帮助企业能够在生产环境中运行和管理Docker和Kubernetes，而无需从头开始构建容器服务平台。Rancher不再维护1.x版本的文档，建议您升级至2.x后，配合2.x文档使用新版Rancher。在任何页面，您都可以单击左上角的logo回到本页。想要下载离线文档，请单击导航栏中的“获取
                        PDF 文档”。
                    </div>
                </div>
            </header>
