...
在**角色**部分中，定义多集群应用的角色。通常，当用户[启动应用商店应用](/docs/catalog/launching-apps/_index)时，该特定用户的权限将用于创建该应用所需的所有工作负载/资源。
...
- **集群** - 这等效于[集群所有者](/docs/admin-settings/rbac/cluster-project-roles/_index)。如果选择此角色，Rancher 将检查所有目标项目中的用户是否具有该项目所在的集群的[集群所有者](/docs/admin-settings/rbac/cluster-project-roles/_index)角色。尽管可能未明确授予下游集群所有者角色，但是如果用户是[系统管理员](/docs/admin-settings/rbac/global-permissions/_index)，则认为该用户具有适当的权限级别。
