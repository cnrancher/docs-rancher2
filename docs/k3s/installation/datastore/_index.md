...
- 如果您希望在边缘部署 Kubernetes，并需要一个高可用的解决方案，但无法承担在边缘管理数据库的操作开销，您可以使用 K3s 建立在 DQLite 之上的嵌入式 HA 数据存储（目前是实验性的）。
...
## 适用于 HA 的嵌入式 DQLite (实验)

K3s 使用 DQLite 类似于使用 SQLite。它的设置和管理都很简单。因此，使用这个选项，不需要进行外部配置或额外的步骤。请参阅[嵌入式数据库的高可用(实验性)](/docs/k3s/installation/ha-embedded/_index)，了解如何使用该选项运行。
