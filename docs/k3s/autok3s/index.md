...
- [K3s离线安装](/docs/k3s/autok3s/airgap/)，K8s离线安装包管理以及使用离线安装方式部署集群。

## 云提供商

AutoK3s 可以支持以下云厂商，我们会根据社区反馈添加更多支持：

- [阿里云](/docs/k3s/autok3s/alibaba/) - 在阿里云的 ECS 中初始化 K3s 集群
- [AWS](/docs/k3s/autok3s/aws/) - 在亚马逊 EC2 中初始化 K3s 集群
- [Google](/docs/k3s/autok3s/google/) - 在Google GCE 中初始化 K3s 集群
- [腾讯云](/docs/k3s/autok3s/tencent/) - 在腾讯云 CVM 中初始化 K3s 集群
- [Native](/docs/k3s/autok3s/native/) - 在任意类型 VM 实例中初始化 K3s 集群
- [K3d](/docs/k3s/autok3s/k3d/) - 使用 K3d 在宿主机 Docker 中初始化 K3s 集群
- [Harvester](/docs/k3s/autok3s/harvester/) - 在 Harvester 实例中初始化 K3s 集群
