...
| `auditLog.level`               | 0                                                                  | `int` - 设置[API 审计日志](/docs/rancher2.5/installation_new/resources/advanced/api-audit-log/_index)等级。0 代表关闭。[0-3] |
...
参阅 [TLS 设置](/docs/rancher2.5/installation_new/resources/tls-settings/_index) 获取更多信息和选项。
...
有关使用私有 registry 安装 Rancher 的详细信息，请参阅[离线环境安装指南](/docs/rancher2.5/installation_new/other-installation-methods/air-gap/_index)。
...
> **注意事项:** 如果您使用的是专用 CA 签名的证书，请添加`--set privateCA=true`并参阅[添加 TLS 密文 - 使用私有的 CA 签名证书](/docs/rancher2.5/installation_new/resources/advanced/helm2/helm-rancher/tls-secrets/_index)来完成给 Rancher 添加 CA 证书。
