...
> 不需要使用外部负载均衡器？
>
> 请看[Rancher 单节点安装指南](/docs/rancher2.5/installation/other-installation-methods/single-node-docker/)

## 关于操作系统、Docker、硬件、网络的需求

确保您的节点满足常规的[安装要求](/docs/rancher2.5/installation/requirements/)。
...
根据 Rancher 的[要求](/docs/rancher2.5/installation/requirements/)设置一台 Linux 主机，以启动 Rancher Server。
...
> 创建自签名证书
>
> - 证书必须是 [PEM 格式](/docs/rancher2.5/installation/other-installation-methods/single-node-docker/troubleshooting/)。
...
- **推荐阅读：** [单节点备份和还原](/docs/rancher2/backups/backup/docker-backups/)。尽管您现在没有任何数据需要备份，但是我们建议您在常规 Rancher 使用之后创建备份。
- [创建 Kubernetes 集群](/docs/rancher2.5/cluster-provisioning/)。
...
如果要使用 Rancher API 记录所有事务，请通过在安装命令中添加以下标志来启用[API 审计](/docs/rancher2.5/installation/resources/advanced/api-audit-log//#开启-api-审计日志)功能。
...
如果要访问此页面以完成[离线安装](/docs/rancher2.5/installation/other-installation-methods/air-gap//)，则在您运行安装命令时，必须在 server 镜像之前添加私有镜像库的 URL。例如在您的`rancher/rancher:latest`前面添加带有您的私有镜像库的 URL`<REGISTRY.DOMAIN.COM:PORT>`。
