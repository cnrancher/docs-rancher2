...
> 注意：
> 可以使用 [一键生成 ssl 自签名证书脚本](/docs/rancher2.5/installation/resources/advanced/self-signed-ssl//#41-一键生成-ssl-自签名证书脚本) 来快速生成符合 rancher 要求的自签名证书。该脚本会自动生成本文中所需要的 `tls.crt`、`tls.key` 和 `cacerts.pem`
...
按照[本页](/docs/rancher2.5/installation/resources/update-ca-cert/)上的步骤更新 Rancher[高可用性 Kubernetes 安装](/docs/rancher2.5/installation/install-rancher-on-k8s/)中的入口的 SSL 证书，或从默认的自签名证书切换到自定义证书。
