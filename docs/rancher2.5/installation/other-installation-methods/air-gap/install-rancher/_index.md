...
## Docker 安装说明
如果你想使用 Docker 离线安装单节点 Rancher Server，请跳过本页的其余部分并按照[本页](/docs/rancher2.5/installation/other-installation-methods/air-gap/install-rancher/docker-install-commands/_index)上的说明进行操作。
...
1. 如果您还没有在可以访问互联网的系统上安装 `helm`，请参考[Helm 版本要求](/docs/rancher2.5/installation/resources/helm-version/_index)来选择一个 Helm 版本来安装 Rancher。
...
> **注意：**如果要在外部终止 SSL/TLS，请参阅[在外部负载均衡器上终止 TLS](/docs/rancher2.5/installation/install-rancher-on-k8s/chart-options/_index#外部-tls-termination)。
...
根据您在[选择您的 SSL 配置](#2-选择您的-ssl-配置)做出的选择，完成以下步骤之一。
