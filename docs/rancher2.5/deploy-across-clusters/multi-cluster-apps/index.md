...
:::note 说明
在 Rancher 2.5.x 中，建议您使用[Fleet](/docs/rancher2.5/deploy-across-clusters/fleet/) 作为跨集群部署应用的工具。
:::
...
[全局应用商店](/docs/rancher2/helm-charts/)中的任何 Helm Chart 均可作为多集群应用来进行部署和管理。

创建多集群应用之后，可以对[全局 DNS](/docs/rancher2/helm-charts/globaldns/) 进行编程，以使其更易于访问该应用。

## 先决条件

要在 Rancher 中创建多集群应用，您必须至少具有以下权限之一：

- 目标集群中的目标项目的[项目成员角色](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)，使您能够创建，读取，更新和删除工作负载。
- 目标项目的集群的[集群所有者角色](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)。
...
在**目标**部分中，选择要在其中部署应用的[项目](/docs/rancher2/cluster-admin/projects-and-namespaces/)。项目列表基于您有权访问哪些项目。对于您选择的每个项目，它将被添加到列表中，其中显示了所选的集群名称和项目名称。要删除目标项目，请单击 **-**。
...
在**角色**部分中，定义多集群应用的角色。通常，当用户[启动应用商店应用](/docs/rancher2/helm-charts/)时，该特定用户的权限将用于创建该应用所需的所有工作负载/资源。
...
- **项目** - 这等效于[项目成员](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)。如果选择此角色，Rancher 将检查在所有目标项目中，用户是否至少具有[项目成员](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)角色。虽然可能未明确授予用户项目成员角色，但如果用户是[系统管理员](/docs/rancher2/admin-settings/rbac/global-permissions/)，或[集群所有者](/docs/rancher2/admin-settings/rbac/global-permissions/)或[项目所有者](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)，则都认为该用户拥有适当的权限级别。

- **集群** - 这等效于[集群所有者](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)。如果选择此角色，Rancher 将检查所有目标项目中的用户是否具有该项目所在的集群的[集群所有者](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)角色。尽管可能未明确授予下游集群所有者角色，但是如果用户是[系统管理员](/docs/rancher2/admin-settings/rbac/global-permissions/)，则认为该用户具有适当的权限级别。
