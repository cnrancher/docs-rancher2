...
有关触发内置告警的详细信息，请参阅[内置告警](/docs/rancher2.5/cluster-admin/tools/cluster-alerts/default-alerts/)。

## 告警示例

- Kubernetes 组件进入不健康状态。
- 节点或者[工作负载](/docs/rancher2/k8s-in-rancher/workloads/)发生错误。
- 部署可以正常被调度。
- 节点的硬件资源过分紧张。

## Prometheus 表达式

> **先决条件：** 监控必须被[启用](/docs/rancher2/cluster-admin/tools/cluster-monitoring/)，您才能使用自定义 Prometheus 查询或表达式触发告警。

当您编辑告警规则时，您将有机会根据 Prometheus 表达式配置要触发的告警。有关表达式的示例，请参考[本页](/docs/rancher2.5/cluster-admin/tools/cluster-monitoring/expression/)。
...
告警包含集群级别和[项目级别](/docs/rancher2/project-admin/tools/project-alerts/)告警
...
作为[集群所有者](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)，您可以配置 Rancher 向您发送有关集群事件的告警。

> **前提：** 在收到集群告警之前，您必须[添加接收者](/docs/rancher2.5/cluster-admin/tools/notifiers/)。
...
1. 根据您要创建的告警类型，查看以下对应操作。
   ...
   - 表达式告警
     ...
     1. 输入或选择一个**表达式**，下拉列表显示 Prometheus 的原始指标表达式，包括：
        ...
        - [**Fluentd**](https://docs.fluentd.org/v1.0/articles/monitoring-prometheus) ([日志](/docs/rancher2/project-admin/tools/project-logging/)中用到的)
...
1. 最后，选择[接收者](/docs/rancher2.5/cluster-admin/tools/notifiers/)。
