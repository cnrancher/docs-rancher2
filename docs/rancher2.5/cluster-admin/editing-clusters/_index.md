...
| 网络提供商           | 为您的集群提供联网功能的[容器联网接口](/docs/rancher2.5/faq/networking/_index)。<br/><br/>**注意：** 您只能在配置集群时选择此选项。以后无法编辑。                                                                                                             |
...
| 节点上的 Docker 版本 | 配置不管节点是否允许运行 Rancher 不正式支持的 Docker 版本。如果您选择需要一个[受支持的 Docker 版本](/docs/rancher2.5/installation_new/resources/advanced/helm2/rke-add-on/layer-7-lb/_index)，Rancher 将停止在没有安装受支持的 Docker 版本的节点上运行 pods。 |
