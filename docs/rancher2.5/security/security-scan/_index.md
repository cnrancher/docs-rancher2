...
> **先决条件：**您需要在配置，发送或接收告警之前配置[通知](/docs/rancher2.5/cluster-admin/tools/cluster-alerts/_index)。
...
1. 为告警选择一个[通知](/docs/rancher2.5/cluster-admin/tools/notifiers/_index)。

**结果：** 告警已创建并激活。当在集群上运行扫描并且满足报警条件时，将触发通知。

有关告警的更多信息，请参考[本页](/docs/rancher2.5/cluster-admin/tools/cluster-alerts/_index)。
