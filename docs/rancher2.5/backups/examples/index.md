...
这个例子表明，如果运行 `rancher-backup` 的节点拥有[这些访问 S3 的权限](./../configuration/back-up-config/#ec2节点访问s3的iam权限设置)，就不必提供 AWS 的凭证 secret 来创建备份。
...
这个例子表明，如果运行 `rancher-backup` 的节点拥有[这些访问 S3 的权限](./../configuration/back-up-config/#ec2节点访问s3的iam权限设置)，就不必提供 AWS 的凭证 secret 来从备份中还原。
