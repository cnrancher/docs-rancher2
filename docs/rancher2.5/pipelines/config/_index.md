...
**通知：** 您可以决定是否要为流水线设置通知。您可以根据流水线的构建状态，启用任何[通知](/docs/rancher2.5/monitoring-alerting/notifiers/_index)。在启用流水线通知之前，您需要[配置通知](/docs/rancher2.5/monitoring-alerting/notifiers/_index)，这样可以轻松地添加接收者。
...
1. 如果您没有任何现有的[通知](/docs/rancher2.5/monitoring-alerting/notifiers/_index)，Rancher 将警告您未设置任何通知，并提供一个链接，可转到设置通知页面。按照[相关说明文档](/docs/rancher2.5/monitoring-alerting/notifiers/_index)添加通知。如果您已有接收者，则可以通过单击**添加接收者**按钮将其添加到通知中。
