...
自我修复节点池，也称为[节点自动替换功能](/docs/rancher2.5/cluster-provisioning/rke-clusters/node-pools/)，是使用 Rancher 设置 vSphere 节点的最大优势。自我修复节点池可以帮助您替换无状态应用程序的工作节点，当您使用 Rancher 通过节点模板创建节点时，Rancher 可以自动替换无法访问的节点。
...
对于要自动获取的字段，您的设置需要满足一些[先决条件](/docs/rancher2.5/cluster-provisioning/rke-clusters/node-pools/vsphere/provisioning-vsphere-clusters/)。
...
在 [本节](/docs/rancher2.5/cluster-provisioning/rke-clusters/node-pools/vsphere/provisioning-vsphere-clusters/) 中，您将学习如何使用 Rancher 在 vSphere 中安装 [RKE](/docs/rke/) Kubernetes 集群。

## 配置存储

有关如何使用 Rancher 在 vSphere 中供应存储的示例，请参阅 [本节](/docs/rancher2.5/cluster-admin/volumes-and-storage/examples/vsphere/)。为了在 vSphere 中动态供应存储，必须 [启用 vSphere provider](/docs/rancher2.5/cluster-provisioning/rke-clusters/cloud-providers/vsphere/)

## 启用 vSphere Cloud Provider

在 Rancher 中设置云提供商时，Rancher 服务器可以自动为集群配置新的基础设施，包括新的节点或持久性存储设备。

有关详细信息，请参阅 [启用 vSphere 云提供商](/docs/rancher2.5/cluster-provisioning/rke-clusters/cloud-providers/vsphere/) 一节。
