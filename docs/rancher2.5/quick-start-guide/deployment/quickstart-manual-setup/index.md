...
1. 请按照[具体要求](/docs/rancher2.5/installation/requirements/)配置 Linux 主机。
...
1.  设置**默认视图**。
    ...
    - 如果选择了 "我想创建或管理多个集群"，则使用 Cluster Manager UI 作为默认视图。
    - 如果选择了 "我只想使用安装在 Rancher 上的集群"，则使用 Cluster Explorer UI 作为默认视图。

1.  设置 **Rancher Server URL**。该 URL 可以是一个 IP 地址，也可以是一个主机名。但是，添加到你的集群中的每个节点必须能够连接到这个 URL。如果你在 URL 中使用一个主机名，这个主机名必须在你想添加到集群的节点上可以通过 DNS 解析。
