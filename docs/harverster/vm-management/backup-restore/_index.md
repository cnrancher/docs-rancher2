---
title: 备份与恢复
description: 虚拟机备份是在**虚拟机**页面上创建的。虚拟机备份卷将存储在备份目标（NFS 或 S3 server），它们可以用来恢复一个新的虚拟机或替换一个现有的虚拟机。
keywords:
  - rancher
  - rancher中文
  - rancher中文文档
...
虚拟机备份是在**虚拟机**页面上创建的。虚拟机备份卷将存储在**备份目标**（NFS 或 S3 server），它们可以用来恢复一个新的虚拟机或替换一个现有的虚拟机。
...
| Certificate        | string | 如果你想使用你的 S3 server 的自签名 SSL 证书，请粘贴该证书 |
