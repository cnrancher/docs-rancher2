...
创建一个[Harvester 配置文件](/docs/harverster/installation/harvester-configuration/_index) `config-create.yaml`用于`CREATE`模式。根据需要修改这些值。
...
创建一个[Harvester 配置文件](/docs/harverster/installation/pxe-mode/_index) `config-join.yaml`用于`JOIN`模式。根据需要修改这些值。
...
关于 Harvester 配置的更多信息，请参考[Harvester 配置](/docs/harverster/installation/pxe-mode/_index)。
