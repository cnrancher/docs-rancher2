...
您可以使用 Rancher 和 GitHub、GitLab 或 Bitbucket 代码仓库集成，配置持续集成（CI）流水线。本文提供了 Rancher 与代码库集成、管理流水线执行配置和配置流水线组件持久存储的操作指导。
...
- **克隆代码：**
  ...
  每次代码合入 GitHub 代码库的时候，流水线会自动克隆一份新的代码，用这份代码构建版本迭代。
...
- **MinIO：**
  ...
  > **说明：** 托管的 Jenkins 实例的工作负载是无状态的，所以您不用担心数据持久化的问题。Docker 镜像仓库和 MinIO 实例默认使用临时存储，临时存储足以应对对大多数情况。如果您希望保证流水线日志在节点失败的情况下能被保存下来，您可以给日志配置持久存储，详情请参考[流水线组件数据持久存储](#配置流水线组件的持久存储)。
...
[Docker 镜像仓库](#流水线的工作原理) 和 [MinIO](#流水线的工作原理) 工作负载默认使用临时存储，临时存储足以应对对大多数情况。如果您希望流水线内嵌的镜像仓库中的镜像和流水线日志在节点失败的情况下能被保存下来，您可以给镜像日志配置持久存储。下文提供了配置持久存储的操作指导。
