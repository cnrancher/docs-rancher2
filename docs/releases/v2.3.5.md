...
- 这次版本发布主要是为了修复 Istio chart 在离线环境中的问题。如果你在使用 v2.3.4 或者 v2.3.5，你将可以使用这个新的 Istio chart。因为只有 v2.3.5 才内嵌了这个新版本 Istio chart，所以如果你是 v2.3.4 的离线环境，并且你正在使用 Rancher Server 内嵌的 System Chart（非手动同步 Rancher System Chart 代码库）的用户，那么你需要升级到 v2.3.5 才能在离线环境下使用 Istio。
