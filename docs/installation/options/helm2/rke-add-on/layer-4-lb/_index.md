...
We will be using NGINX as our Layer 4 Load Balancer (TCP). NGINX will forward all connections to one of your Rancher nodes. If you want to use Amazon NLB, you can skip this step and use [Amazon NLB configuration](/docs/installation/k8s-install-server-install/nlb/)
...
1. Download one of following templates, depending on the SSL certificate you're using.
   ...
   >
   > - Want records of all transactions with the Rancher API? Enable the [API Auditing](/docs/installation/api-auditing) feature by editing your RKE config file. For more information, see how to enable it in [your RKE config file](/docs/installation/options/helm2/rke-add-on/api-auditing/).
...
- Create a backup of your Rancher Server in case of a disaster scenario: [High Availability Back Up and Restoration](/docs/installation/backups-and-restoration/ha-backup-and-restoration).
