...
为了利用这个功能，在[Kubernetes API 服务配置](/docs/rke/config-options/services/)中添加了一个新的字段`secrets_encryption_config`。一个完整的自定义配置是这样的：
...
因此，我们建议，当您启用或禁用加密时，或者当您旋转密钥时，您应该[创建快照](/docs/rke/etcd-snapshots/one-time-snapshots/)，以便您的备份需要您可以访问的相同密钥。
