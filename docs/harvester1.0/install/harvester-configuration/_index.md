...
> 如需为 Harvester 组件（如获取外部镜像和备份到 S3）配置 HTTP(S) 代理，请参见 [Settings/http-proxy](../../settings/settings/#http-proxy)。
...
不创建额外的磁盘分区来存储虚拟机数据。
虚拟机数据将存储在操作系统的分区中。
如果你想使用额外的磁盘来存储虚拟机数据，[auto-disk-provision-paths](../../settings/settings/#auto-disk-provision-paths) 设置就非常有用。
...
有关其他信息和所有选项的列表，请参见[设置](../../settings/settings)页面。
