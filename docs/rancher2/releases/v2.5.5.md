...
- Rancher 的集群浏览器现在只在 Windows 集群上显示 Windows 兼容的图表。
...
- 监控
  ...
  - 为了继续支持使用 9.4.200 版本 Rancher 监控图的用户，Dashboard UI 设置的 k3s 集群的默认内存限制已经增加到 2500Mi。对于升级到图表 9.4.201 版本的用户来说，不需要这个增加的限制，但建议使用。
