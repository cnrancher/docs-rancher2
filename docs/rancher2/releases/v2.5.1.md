...
Rancher 2.5.1 版本于 2020 年 10 月 8 日发布。点击[这里](https://github.com/rancher/rancher/releases/tag/v2.5.1)查看英文版版本说明。
由于 Dockerhub 上的 rancher-agent 镜像损坏，Rancher 2.5.1 是 2.5.0 的镜像版本，但使用了重建的镜像。这个版本中没有引入任何 bug 修复或增强功能，版本说明与 2.5.0 相同。

## 重要说明

自 2.0 版本以来，Rancher 中的主要 UI 现在称为 Cluster Manager。我们新的 Cluster Explorer 仪表板，在 Rancher 2.4 中实验性地发布，已经升级到 GA 状态。有一些新的功能只在新的 Cluster Explorer 仪表板中可用。这些新功能中的一些功能与 Cluster Manager 中的现有功能类似，我们将尝试根据它们在 UI 中的位置来区分它们。
...
- 本地群集不能再关闭，这意味着所有管理员都可以访问本地群集。如果你想限制本地集群的权限，必须使用一个新的 restricted-admin role。
...
| Cluster Manager - Rancher 日志       | 使用簇集群资源管理器中的应用程序和市场中可用的新的日志，用新的日志解决方案替换功能。                                                           |
...
- 增强的 EKS 生命周期管理：EKS 供应已得到增强，支持受管节点组、私有访问和控制平面日志。用户现在可以将使用其他工具（如 eksctl）配置的现有 EKS 群集注册到 Rancher 中，以便对升级和配置进行管理。增加了对具有异构配置的多个受管节点组的支持，以便在同一集群中实现 GPU 和非 GPU 工作负载。Rancher 中的 EKS 集群配置现在使用云凭证，将 AWS 密钥管理与集群配置解耦。
...
- Rancher 备份
  - 当迁移到具有 Rancher 备份功能的群集时，服务器-url 不能更改到不同的位置，必须继续使用相同的 URL。
