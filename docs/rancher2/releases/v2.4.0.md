...
- **性能优化 [[#23013](https://github.com/rancher/rancher/issues/23013)]**：单个 Rancher control-plane 现在最多可管理两千个集群或十万个节点。Rancher 通过使用共享的全局集群状态，减少了管理每个集群所需的内存空间和 Kubernetes API 连接数。并优化了 Rancher 控制器，以减少对 Kubernetes API 的调用总数。基于这些改进，我们更新了用于安装 Rancher 的[集群规模指南](/docs/rancher2/installation/requirements/)。
