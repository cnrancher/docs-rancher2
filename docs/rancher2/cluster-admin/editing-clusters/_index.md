...
| Nginx Ingress        | 如果你想在高可用性配置中发布你的应用程序，并且你的节点是由一个没有本地负载平衡功能的云提供商托管的，那么启用这个选项可以在集群中使用 Nginx ingress。    |
...
| 节点上的 Docker 版本 | 配置不管节点是否允许运行 Rancher 不正式支持的 Docker 版本。如果您选择需要一个[受支持的 Docker 版本](/docs/rancher2/installation_new/resources/advanced/helm2/rke-add-on/layer-7-lb/_index)，Rancher 将停止在没有安装受支持的 Docker 版本的节点上运行 pods。 |
