...
因此您可以配置 Prometheus 去收集集群级别或者项目级别的监控数据。本章节将介绍如何启用对集群的监控。有关对项目的监控，可以浏览[项目管理部分](/docs/rancher2/cluster-admin/tools/cluster-monitoring/project-monitoring/)。
...
通过 Prometheus，您可以在 Rancher 上在集群级别和[项目级别](/docs/rancher2/cluster-admin/tools/cluster-monitoring/project-monitoring/)进行监控。对于每个启用了监控的集群和项目，Rancher 都会部署一个 Prometheus 服务。

- 集群监控可让您查看 Kubernetes 集群的运行状况。Prometheus 从下面的集群组件中收集指标，您可以在 Chart 中查看这些指标。

  - [Kubernetes Control Plane](/docs/rancher2/cluster-admin/tools/cluster-monitoring/cluster-metrics/)
  - [etcd 数据库](/docs/rancher2/cluster-admin/tools/cluster-monitoring/cluster-metrics/)
  - [所有节点](/docs/rancher2/cluster-admin/tools/cluster-monitoring/cluster-metrics/)

- [项目监控](/docs/rancher2/cluster-admin/tools/cluster-monitoring/project-monitoring/)允许您查看在具体某个项目内运行的 Pods 的状态。项目级别的 Prometheus 可以从本项目内部署的工作负载中采集自定义指标，这些工作负载必须通过 HTTP 和 TCP/UDP 暴露指标。

## 启用集群监控

作为[系统管理员](/docs/rancher2/admin-settings/rbac/global-permissions/)或[集群所有者](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)，您可以通过配置来监控您的 Kubernetes 集群。
...
> 要在 Kubernetes v1.21 以上版本上安装监控，您需要[迁移到监控 V2。](/docs/rancher2.5/monitoring-alerting/guides/migrating/)

1. 在**全局**页面中导航到您想要配置的集群。
1. 在导航栏中下拉**工具**，选择**监控**。
1. 查看[资源消耗建议](#资源消耗)，以确保您有足够的资源用于 Prometheus 及其相关组件。根据需要，配置 [Prometheus 选项](/docs/rancher2/cluster-admin/tools/cluster-monitoring/expression/)。
1. 单击**启动**。

**结果：**将部署 Prometheus 服务以及两个监控[应用商店应用](/docs/rancher2/helm-charts/)。这两个监控应用商店应用是`cluster-monitoring`和`monitoring-operator`，它们会被添加到集群的`系统（System）`项目中。当这两个应用处于`Active`后，您可以通过 [Rancher 集群仪表盘](/docs/rancher2/cluster-admin/tools/cluster-monitoring/viewing-metrics/)开始查看[集群指标](/docs/rancher2/cluster-admin/tools/cluster-monitoring/cluster-metrics/)或直接从 [Grafana](/docs/rancher2/cluster-admin/tools/cluster-monitoring/)中查看。
