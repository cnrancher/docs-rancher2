...
[迁移工具 CLI](/docs/rancher2/v1.6-migration/run-migration-tool/) 无法将健康检查从 Compose 文件解析为 Kubernetes 清单。因此，如果要向 Rancher v2.x 工作负载添加健康检查，则必须手动添加它们。
...
- TCP 检查
  ...
  您可以通过选择**健康检查**部分中的**TCP 连接成功打开**选项来配置探针以及指定对应行为的值。有关更多信息，请参阅[部署工作负载](/docs/rancher2/k8s-in-rancher/workloads/deploy-workloads/)。有关设置探针超时和阈值的帮助，请参见[健康检查参数映射](#健康检查参数映射)。
...
- HTTP 检查
  ...
  您可以通过选择**HTTP 返回成功状态**或**HTTPS 返回成功状态**来配置探针以及用于指定对应行为的值。有关更多信息，请参见[部署工作负载](/docs/rancher2/k8s-in-rancher/workloads/deploy-workloads/)。有关设置探针超时和阈值的帮助，请参见[健康检查参数映射](#健康检查参数映射)。
...
您可以配置活性检查或就绪检查，以执行指定的命令，方法是在[部署工作负载](/docs/rancher2/k8s-in-rancher/workloads/deploy-workloads/)时，从 **健康检查** 中选择`容器中进程退出状态码检查(0)`。
...
## [下一步: 调度服务](/docs/rancher2/v1.6-migration/schedule-workloads/)
