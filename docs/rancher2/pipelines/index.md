...
> - 还在使用 v2.0.x 版本吗? 请参阅[早期版本](/docs/rancher2/k8s-in-rancher/pipelines/docs-for-v2.0.x/)的流水线配置文档。
> - Rancher 的流水线提供了简单的持续集成\持续交付（CI/CD）体验，但是它是简化版的流水线，并不能提供典型流水线的全部功能。Rancher 的流水线不是一个企业级 Jenkins 或其他企业级持续集成工具的替代品。

## 概念

有关本节中使用的概念和术语的说明，请参阅[本页](/docs/rancher2/k8s-in-rancher/pipelines/concepts/)。
...
在设置任何流水线之前，请确保此项目已经对接了您的版本管理工具，例如，GitHub，GitLab，Bitbucket。如果您没有配置版本管理工具，您可以使用[Rancher 提供的示例代码库](/docs/rancher2/k8s-in-rancher/pipelines/example-repos/)去预览一些常见的流水线部署流程。
...
- **Minio：**
  ...
  > **说明：** 托管的 Jenkins 实例的工作负载是无状态的，所以您不用担心数据持久化的问题。Docker 镜像仓库和 Minio 实例默认使用临时存储，临时存储足以应对对大多数情况。如果您希望保证流水线日志在节点失败的情况下能被保存下来，您可以给日志配置持久存储，详情请参考[流水线组件数据持久存储](/docs/rancher2/k8s-in-rancher/pipelines/storage/)。

## 流水线的基于角色的访问控制

如果您可以访问一个项目，您就能激活代码库来构建流水线。

但只有[系统管理员](/docs/rancher2/admin-settings/rbac/global-permissions/)，[集群所有者或集群成员](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)，或者[项目所有者](/docs/rancher2/admin-settings/rbac/cluster-project-roles/)能够授权版本管理工具并修改其他项目层级的流水线参数设置。
...
**结果：** 完成版本控制工具提供商认证后，Rancher 会重新导向到开始[配置代码库](/docs/rancher2/k8s-in-rancher/pipelines/)的页面。启用代码库后，您可以开始[配置流水线](/docs/rancher2/k8s-in-rancher/pipelines/)。
...
1. 通过 UI 或者使用代码库中的 yml 文件，例如 `.rancher-pipeline.yml` 或者 `.rancher-pipeline.yaml`进行流水线的配置。流水线的配置分为`阶段`和`步骤`。在进入下一个`阶段`之前，当前`阶段`必须全部完成，但是一个`阶段`中的`步骤`是同时运行的。对于每个阶段，您可以添加不同的步骤类型。注意：在构建每个步骤时，根据步骤类型有不同的高级选项。高级选项包括触发规则，环境变量和密文。有关通过 UI 或 YAML 文件配置流水线的更多信息，请参考[流水线配置参考](/docs/rancher2/k8s-in-rancher/pipelines/config/)。
...
请参考[流水线配置参考](/docs/rancher2/k8s-in-rancher/pipelines/config/)，了解更多关于如何配置流水线的细节。
...
在此第一次运行流水线的时候，Rancher 会将以下[流水线组件](/docs/rancher2/project-admin/pipelines/)作为工作负载部署到项目的新命名空间中，作为专用于流水线的工作负载:
...
> **注意：** 对于 Rancher 的[示例代码库](/docs/rancher2/k8s-in-rancher/pipelines/example-repos/)，流水线的`设置`项不存在。
