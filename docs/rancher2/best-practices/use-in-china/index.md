...
- `system-default-registry:` 参数请参考[官方文档](/docs/rancher2/admin-settings/config-private-registry/)

1. 接下来我们就可以按照官方文档去[添加自定义集群](/docs/rancher2/cluster-provisioning/rke-clusters/custom-nodes/)，我们只需要等待集群启动成功即可。下图中列出了 system-project 中所有的 workload，这些 workload 均使用了阿里云的镜像仓库`registry.cn-hangzhou.aliyuncs.com`内的镜像
...
要解决这个问题只需要参考[获取新的 Kubernetes 版本](/docs/rancher2.5/admin-settings/k8s-metadata/)将 rke-metadata-config 地址修改成[gitee](https://gitee.com/rancher/kontainer-driver-metadata/)上的地址即可，例如：
...
在国内使用 helm 安装 rancher，[添加 Helm Chart 仓库](/docs/rancher2/installation/resources/advanced/helm2/helm-rancher/)时有时候会因为访问国外网络导致 Rancher Chart 添加失败，如下:
