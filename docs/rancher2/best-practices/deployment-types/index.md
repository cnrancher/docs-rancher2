...
建议使用托管的基础设施，如 Amazon 的 EC2 或谷歌的 GCE。在基础设施提供者上使用 RKE 创建集群时，您可以配置集群创建 etcd 快照作为备份。然后，您可以使用 [RKE](/docs/rke/etcd-snapshots/recurring-snapshots/) 或 [Rancher](/docs/rancher2/backups/restore/) 从这些快照之一恢复您的集群。在托管的 Kubernetes 环境中，不支持这种备份和恢复功能。

## 确保 Kubernetes 的节点配置正确

当您部署节点时需要遵循 Kubernetes 和 etcd 最佳实践，比如：禁用 swap、反复检查集群中的所有机器之间的网络连接、使用唯一的主机名、使用唯一的 MAC 地址、使用唯一的 product_uuids、检查所有需要的端口被打开，部署使用 ssd 的 etcd。更多的细节可以在 [Kubernetes 文档](https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/install-kubeadm) 和 [etcd 的性能操作指南](https://github.com/etcd-io/website/tree/master/content/en/docs) 中找到。
...
Rancher Server 的 Local Kubernetes 集群应该尽可能符合[系统和硬件需求](/docs/rancher2/installation/requirements/)。您越偏离系统和硬件需求，您承担的风险就越大。
...
在集群中[启用监控](/docs/rancher2/cluster-admin/tools/cluster-monitoring/)之后，您可以设置[通知](/docs/rancher2/cluster-admin/tools/notifiers/)和[告警](/docs/rancher2/cluster-admin/tools/cluster-alerts/)，让您知道您的集群是否接近其容量。您还可以使用 Prometheus 和 Grafana 监控框架来建立适合您的规模的关键指标基准。
