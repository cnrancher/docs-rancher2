...
> **如何配置单节点安装的外部负载均衡？**
>
> 请参阅[使用外部负载均器的单节点安装](/docs/rancher2/installation/options/single-node-install-external-lb/_index)。
...
- **推荐：** 查看[单节点备份和还原](/docs/rancher2/backups/backups/single-node-backups/_index)。尽管您现在没有任何数据需要备份，但是我们建议您在常规使用 Rancher 之后创建备份。
