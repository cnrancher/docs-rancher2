...
根据我们的[要求](/docs/rancher2/installation/requirements/)配置三台 Linux 主机。
...
- [配置 Amazon ALB](/docs/rancher2/installation/resources/advanced/helm2/rke-add-on/layer-7-lb/alb/)
- [配置 NGINX](/docs/rancher2/installation/resources/advanced/helm2/rke-add-on/layer-7-lb/nginx/)
...
1. 下载以下模板之一，具体取决于您使用的 SSL 证书。
   ...
   - [由公认的 CA 签署的证书模板 `3-node-certificate-recognizedca.yml`](/docs/rancher2/installation/resources/advanced/cluster-yml-templates/node-certificate-recognizedca/)
     > **高级配置选项：**
     >
     > - 想要记录 Rancher API 的所有事务? 通过编辑 RKE 配置文件来启用[API 审计日志](/docs/rancher2/installation/resources/advanced/api-audit-log/)功能。有关更多信息，请参见如何在[RKE 配置文件中](/docs/rancher2/installation/resources/advanced/api-audit-log/)中启用它。
...
1.  使用您的 Linux 主机信息更新配置模板中的 `nodes` 域。
    ...
    > **注意：**
    >
    > 使用 RHEL / CentOS 时，由于https://bugzilla.redhat.com/show_bug.cgi?id=1527565 导致 SSH 用户无法成为 root 用户。有关 RHEL / CentOS 的特定要求，请参阅[操作系统](/docs/rke/os/)要求。
...
> - 证书文件必须为 `PEM` 格式。
> - 证书文件必须使用 `Base64` 编码。
> - 在您的证书文件中，包括链中的所有中间证书。顺序为前面为证书，后面跟着其他中间证书。有关示例，请参阅[常见问题/故障排查](/docs/rancher2/installation/resources/advanced/helm2/rke-add-on/troubleshooting/)。
...
- **推荐：** 查看[创建备份：高可用性备份和还原](/docs/rancher2/backups/backup/ha-backups/)，以了解在灾难情况下如何备份 Rancher Server。
- 创建 Kubernetes 集群： [创建集群](/docs/rancher2/cluster-provisioning/)。
