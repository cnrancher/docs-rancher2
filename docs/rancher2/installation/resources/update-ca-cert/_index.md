...
请按照这些步骤更新 Rancher[高可用 Kubernetes 安装](/docs/rancher2/installation/install-rancher-on-k8s/_index)中的入口的 SSL 证书，或者从默认的自签名证书切换到自定义证书。
...
如果证书是由私有 CA 签署的，也要添加`set privateCA=true`参数。另外，请务必阅读描述使用[自定义证书](/docs/rancher2/installation/install-rancher-on-k8s/_index)进行初始安装的文档。
