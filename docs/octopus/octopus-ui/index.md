...
默认情况下，`Octopus-UI`在 Octopus[Helm Chart](../install/)安装时会自动部署，您始终可以使用以下命令将其打开或关闭：
...
`Octopus-UI`使用 k3s 生成的用户名和密码进行身份验证，您可以从生成的 k3s[[KUBECONFIG]](/docs/k3s/cluster-access/)文件中找到它。
